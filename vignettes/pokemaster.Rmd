...
- The first function is **pokedex**. It is a function that creates dataframe with features like id, name, stats and types. The default output of the function is a dataframe of all Pokémons, but the user can retrieve the information of only the Pokémons of interest.
...
- The third function is **pokecard**, which generates a flashcard of the team. It takes the input of vector of up to 6 Pokemons and has an output of an image/collage with the sprites of the Pokémons. 

- The forth function is **pokestats**. It is a function that plots barplot of the basic stats for each Pokémon. It take an input vector of up to 6 Pokémons, and generates a barplot. We provide visualization for hitpoints (HP), Attack, Defense, Special Attack, Special Defense and Speed.
